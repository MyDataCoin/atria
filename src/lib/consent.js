...
//
// Ручка на бэкенде есть: POST /api/v1/consent (ConsentController, [Authorize]) — сохраняет
// кто/тип/версию/когда и идемпотентна по (пользователь, тип, версия). Принятие ПДн текущей
// версии — предусловие для POST /kyc/submit, поэтому вызов обязан быть блокирующим: без
// записи согласия персональные данные в обработку не уходят.
